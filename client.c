...
//Validate if command is valid inputted by the user
//Command Type is returned:
// 1 - findFile
// 2 - sgetfiles
// 3 - dgetfiles
// 4 - getfiles
// 5 - gettargz
// 6 - exit
int validateCommandAndReturnType(char *rawCommand) {
...
int connectToServer(char *host, char *port) {
    ...
    if (connect(server, (struct sockaddr *) &servAdd, sizeof(servAdd)) < 0) {
        fprintf(stderr, "connect to server failed, Exiting...\n");
        exit(3);
    }
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        int type = validateCommandAndReturnType(copy);
        if (type == -1) {
            continue;
        }
