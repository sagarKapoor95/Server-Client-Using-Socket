...
//Switch To Mirror
void switchToMirror(int descriptor, char *mirrorIp) {
    printf("Switching to mirror %s for client %d\n", mirrorIp, descriptor);
    char buffer[100];
    int n = sprintf(buffer, "%s", mirrorIp);
    buffer[n] = '\0';
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 3) {
        fprintf(stderr, "Invalid Server start. <ServerFilePath> <Port> <MirrorIP>\n");
        exit(0);
    }
    ...
    while (1) {
        ...
        numOfConnections++;
        if ((numOfConnections > 4 && numOfConnections < 9) || (numOfConnections > 8 && numOfConnections % 2 == 0)) {
            switchToMirror(clientDescriptor, mirrorIp);
            continue;
        }
